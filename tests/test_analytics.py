...
def test_get_usage(client):
    with requests_mock.Mocker() as m:
        m.get("https://api.unsent.dev/v1/analytics", json={"sent": 100, "limit": 1000}, status_code=200)
...
def test_get_daily_stats(client):
    with requests_mock.Mocker() as m:
        m.get("https://api.unsent.dev/v1/analytics/time-series?startDate=2023-01-01&endDate=2023-01-31", json=[{"date": "2023-01-01", "sent": 10}], status_code=200)
...
def test_get_domain_reputation(client):
    with requests_mock.Mocker() as m:
        m.get("https://api.unsent.dev/v1/analytics/reputation?domainId=dom1", json={"domain": "example.com", "reputation": "high"}, status_code=200)
